...
bool AttributeHelper::putAttributeInAtenAdd(layer_inID_type& layer_inID, dtensor_ptr_type& d_tensor) {
    ...
    if (idx == 0 || idx == 1) {
        Log::IR::D() << "prim::Constant attempts to set into the input of layer: " << layer_inID.first->getName();
        return false;
    } else if (idx == 2) {
        cur_layer->setAlpha(getValueFromConstant<int64_t>(d_tensor, cur_layer->getType(), "alpha"));
    } else {
        Log::IR::E() << "Incorrect data from prim::Constant";
    }
    return true;
}
...
bool AttributeHelper::putAttributeInAtenArange1(layer_inID_type& layer_inID, dtensor_ptr_type& d_tensor) {
    ...
    if (idx == 0) {
        ...
        if ((*data).size()) {
            ...
            Log::IR::I() << "dtype of aten::arange is set to NONE";
            return false;
        }
    } else if (idx == 2) {
        auto data = d_tensor->getData<int64_t>();
        if ((*data).size()) {
            ...
            Log::IR::I() << "layout of aten::arange is set to NONE";
            return false;
        }
    } else if (idx == 3) {
        ...
        if ((*data).size()) {
            ...
            Log::IR::I() << "pin_memory of aten::arange is set to NONE";
            return false;
        }
    } else {
        Log::IR::E() << "Incorrect data from prim::Constant";
...
bool AttributeHelper::putAttributeInAtenAsTensor(layer_inID_type& layer_inID, dtensor_ptr_type& d_tensor) {
    ...
    if (idx == 0) {
        ...
        if ((*data).size()) {
            ...
            Log::IR::I() << "dtype of aten::as_tensor is set to NONE";
            return false;
        }
    } else if (idx == 2) {
        cur_layer->setDevice(getStringFromConstant(d_tensor, cur_layer->getType(), "device"));
...
bool AttributeHelper::putAttributeInAtenBatchNorm(layer_inID_type& layer_inID, dtensor_ptr_type& d_tensor) {
    ...
    if (idx == 0) {
        ...
        Log::IR::I() << "weights of aten::batch_norm have been set in layer builder stage";
    } else if (idx == 2) {
        Log::IR::I() << "bias of aten::batch_norm have been set in layer builder stage";
    } else if (idx == 3) {
        Log::IR::I() << "running_mean of aten::batch_norm have not been set in layer builder stage, don't delete";
        return false;
    } else if (idx == 4) {
        Log::IR::I() << "running_var of aten::batch_norm have not been set in layer builder stage, don't delete";
        return false;
    } else if (idx == 5) {
...
bool AttributeHelper::putAttributeInAtenConv2d(layer_inID_type& layer_inID, dtensor_ptr_type& d_tensor) {
    ...
    if (idx == 0) {
        ...
        Log::IR::I() << "weights of aten::conv2d have been set in layer builder stage";
    } else if (idx == 2) {
        Log::IR::I() << "bias of aten::conv2d have been set in layer builder stage";
    } else if (idx == 3) {
...
bool AttributeHelper::putAttributeInAtenEmbedding(layer_inID_type& layer_inID, dtensor_ptr_type& d_tensor) {
    ...
    if (idx == 0) {
        Log::IR::I() << "weights of aten::embedding will be processed in later pass.";
        return false;
    } else if (idx == 1) {
        Log::IR::D() << "prim::Constant attempts to set into the input of layer: " << layer_inID.first->getName();
        return false;
...
bool AttributeHelper::putAttributeInAtenLogSoftmax(layer_inID_type& layer_inID, dtensor_ptr_type& d_tensor) {
    ...
    if (idx == 0) {
        ...
        if ((*data).size()) {
            ...
            Log::IR::I() << "dtype of aten::log_softmax is set to NONE";
            return false;
        }
    } else {
        Log::IR::E() << "Incorrect data from prim::Constant";
...
bool AttributeHelper::putAttributeInAtenLstm1(layer_inID_type& layer_inID, dtensor_ptr_type& d_tensor) {
    ...
    if (idx == 0 || idx == 1) {
        ...
        Log::IR::I() << "weights of aten::lstm1 have been set in layer builder stage";
    } else if (idx == 3) {
...
bool AttributeHelper::putAttributeInAtenLstm2(layer_inID_type& layer_inID, dtensor_ptr_type& d_tensor) {
    ...
    if (idx == 0 || idx == 1 || idx == 2) {
        ...
        Log::IR::I() << "weights of aten::lstm2 have been set in layer builder stage";
    } else if (idx == 4) {
        cur_layer->setHasBiases(getValueFromConstant<int>(d_tensor, layer_type, "has_biases"));
    } else if (idx == 5) {
        cur_layer->setNumLayers(getValueFromConstant<int64_t>(d_tensor, layer_type, "num_layers"));
...
bool AttributeHelper::putAttributeInAtenOnes(layer_inID_type& layer_inID, dtensor_ptr_type& d_tensor) {
    ...
    if (idx == 0) {
        ...
        if ((*data).size()) {
            ...
            Log::IR::I() << "dtype of aten::ones is set to NONE";
            return false;
        }
    } else if (idx == 2) {
        auto data = d_tensor->getData<int64_t>();
        if ((*data).size()) {
            ...
            Log::IR::I() << "layout of aten::ones is set to NONE";
            return false;
        }
    } else if (idx == 3) {
        cur_layer->setDevice(getStringFromConstant(d_tensor, cur_layer->getType(), "device"));
        ...
        if ((*data).size()) {
            ...
            Log::IR::I() << "pin_memory of aten::ones is set to NONE";
            return false;
        }
    } else {
        Log::IR::E() << "Incorrect data from prim::Constant";
...
bool AttributeHelper::putAttributeInAtenSoftmax(layer_inID_type& layer_inID, dtensor_ptr_type& d_tensor) {
    ...
    if (idx == 0) {
        ...
        if ((*data).size()) {
            ...
            Log::IR::I() << "dtype of aten::softmax is set to NONE";
            return false;
        }
    } else {
        Log::IR::E() << "Incorrect data from prim::Constant";
...
bool AttributeHelper::putAttributeInAtenSum(layer_inID_type& layer_inID, dtensor_ptr_type& d_tensor) {
    ...
    if (idx == 0) {
        ...
        if ((*data).size()) {
            ...
            Log::IR::I() << "dtype of aten::sum is set to NONE";
            return false;
        }
    } else {
        Log::IR::E() << "Incorrect data from prim::Constant";
...
bool AttributeHelper::putAttributeInAtenTo1(layer_inID_type& layer_inID, dtensor_ptr_type& d_tensor) {
    ...
    if (idx == 0) {
        ...
        if ((*data).size()) {
            ...
            Log::IR::I() << "NONE is set to attribute optional_memory_format of aten::to";
        }
    } else {
        Log::IR::E() << "Incorrect data from prim::Constant";
    }

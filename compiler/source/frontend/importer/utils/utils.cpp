...
#define SPLIT(name) Log::NIR::I() << "*****" << name << "*****";
...
int TorchscriptPrinter::printGraphRecursive(std::string filename)
{
    ...
    for (auto m : script_model.named_modules()) {
        ...
        Log::NIR::I() << "submodule_name:" << module_name;
        for (auto f : submodule.get_methods()) {
            SPLIT_GRAPH(ret + 1);
            Log::NIR::I() << "method_name:" << f.name();
            Log::NIR::I() << f.graph()->toString(true);
            ret++;
        }
    }
    return ret;
...
void TorchscriptPrinter::printNodeRecursive(torch::jit::Node *node, size_t level)
{
...
#if PRINT_INFO
    Log::NIR::I() << "scope_name:" << scope_name;
    Log::NIR::I() << "node_name:" << node_name;
#endif

    // Get the inputs of node
    SPLIT("Inputs");
    int num_inputs = node->inputs().size();
    if (num_inputs == 0) Log::NIR::I() << "None";

    for (size_t i = 0; i < node->inputs().size(); i++) {
        // input type & name
        std::string input_type = node->inputs().at(i)->type()->str();
        std::string input_debug_name = node->inputs().at(i)->debugName();
        Log::NIR::I() << "input_type:" << input_type;
        Log::NIR::I() << "input_debug_name:%" << input_debug_name;
    }

    // Get the outputs of node
    SPLIT("Outputs");
    int num_outputs = node->outputs().size();
    if (num_outputs == 0) Log::NIR::I() << "None";
#if PRINT_INFO
    Log::NIR::I() << "num_outputs:" << num_outputs;
#endif
...
#if PRINT_INFO
        Log::NIR::I() << "output_type:" << output_type;
        Log::NIR::I() << "output_debug_name: %" << output_debug_name;
#endif
    }
    ...
    if (num_attrs == 0) Log::NIR::I() << "None";
    Log::NIR::I() << "num_attrs:" << num_attrs;
    for (auto attr_name : node->attributeNamesS()) {
#if PRINT_INFO
        Log::NIR::I() << "attr_name:" << attr_name;
#endif
    }
    // the body of node
    // Recursive
...
void TorchscriptPrinter::printScriptModelRecursive(std::string filename)
{
    ...
    for (auto m : script_model.named_modules()) {
        ...
        for (auto f : submodule.get_methods()) {
            ...
            Log::NIR::I() << "num_inputs_graph: " << inputs.size();
            // Get the type of each input
            for (size_t i = 0; i < inputs.size(); i++) {
                Log::NIR::I() << "type: " << inputs.at(i)->type()->str();
            }

            // Get the Outputs of Graph
            auto outputs = graph->outputs();
            Log::NIR::I() << "num_outputs_graph: " << outputs.size();
            // Get the type of each output
            for (size_t i = 0; i < outputs.size(); i++) {
                Log::NIR::I() << "type: " << *outputs.at(i)->type();
            }

...
std::shared_ptr<ir::NNLayer> AtenLSTM2Builder::buildLayer(const torch::jit::Node* node_ref)
{
    ...
    const torch::jit::Node* node_lstm2 = node_ref;

    // get the weights/bias from jit::Node and add to layer
    std::vector<DTensor> weights;
    std::vector<DTensor> biases;
    getLearnableParameters(node_lstm2, weights, biases);
    aten_lstm2_layer_->setWeights(weights);
    aten_lstm2_layer_->setBiases(biases);
 ...
 */
void AtenLSTM2Builder::ptTensor2nncompilerDTensor(at::Tensor torch_tensor, DTensor& d_tensor)
{
 ...
 */
void AtenLSTM2Builder::getLearnableParameters(const torch::jit::Node* node_lstm, std::vector<DTensor>& weight_tensors,

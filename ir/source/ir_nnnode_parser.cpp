...
IRNNNodeParser::parseNNNode<IR::NNNode::AnyType_AtenEmbeddingNode>(const IR::NnNode*      ir_node,
    ...
    auto weights_fp32 = makeDataArrFromVector<float>(aten_embedding_node->weights());
    std::vector<float16> weights;
    for (auto item : weights_fp32) {
        weights.push_back(static_cast<float16>(item));
    }
    nn_ir::Shape2D weights_shape = std::get<nn_ir::Shape2D>(nn_ir::parseParam(aten_embedding_node->weights_shape()));
    ...
    return std::make_unique<nn_ir::AtenEmbeddingNode>(node_info, weights, weights_shape, padding_idx,
...
template <>
std::unique_ptr<nn_ir::NNNode>
IRNNNodeParser::parseNNNode<IR::NNNode::AnyType_AtenLSTM1Node>(const IR::NnNode*      ir_node,
                                                              const nn_ir::NodeInfo& node_info) {
    ...
    auto weight_blob_id = makeDataArrFromVector<int64_t>(aten_lstm1_node->weight_blob_ids());
...
template <>
std::unique_ptr<nn_ir::NNNode>
IRNNNodeParser::parseNNNode<IR::NNNode::AnyType_AtenLSTM2Node>(const IR::NnNode*      ir_node,
                                                              const nn_ir::NodeInfo& node_info) {
    ...
    auto weight_blob_id = makeDataArrFromVector<int64_t>(aten_lstm2_node->weight_blob_ids());
    ...
    const IR::NnNode* ir_node, const nn_ir::NodeInfo& node_info)
{
    ...
    auto is_inplace = aten_masked_fill_node->is_inplace();
    Log::IR::E_IF(aten_masked_fill_node == nullptr)
        << "IRNNNodeParser::parseNNNode<NN::AtenMaskedFillNode>() => wrong node type!";
    return std::make_unique<nn_ir::AtenMaskedFillNode>(node_info, is_inplace);
}
...
template <>
std::unique_ptr<nn_ir::NNNode> IRNNNodeParser::parseNNNode<IR::NNNode::AnyType_AtenNormNode>(
    const IR::NnNode* ir_node, const nn_ir::NodeInfo& node_info)
{
    auto aten_norm_node = ir_node->nn_node_as_AtenNormNode();
    Log::IR::E_IF(aten_norm_node == nullptr) << "IRNNNodeParser::parseNNNode<NN::AtenNormNode>() => wrong node type!";

    auto p = aten_norm_node->p();
    return std::make_unique<nn_ir::AtenNormNode>(node_info, p);
}

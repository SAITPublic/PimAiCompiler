...
class PrimVariableNode : public NodeMixin<PrimVariableNode, CONTROLNode> {
 public:
    explicit PrimVariableNode(const NodeInfo &node_info, std::vector<uint8_t> data, std::vector<Shape4D> shape,
                              std::string data_type, std::vector<std::string> tensor_data_type)
            : NodeMixin(node_info, NodeType::PRIMVARIABLE), data_(data), shape_(shape),
              data_type_(data_type), tensor_data_type_(tensor_data_type) {}

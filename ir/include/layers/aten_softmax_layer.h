...
class AtenSoftmaxLayer : public NNLayer
{
   public:
    ...
    AtenSoftmaxLayer(std::string name, LayerType type) : NNLayer(name, type) {}

    explicit AtenSoftmaxLayer(const AtenSoftmaxLayer& aten_softmax_layer) : NNLayer(aten_softmax_layer)
    {
    ...
    virtual std::shared_ptr<NNLayer> clone() { return std::shared_ptr<AtenSoftmaxLayer>(new AtenSoftmaxLayer(*this)); }

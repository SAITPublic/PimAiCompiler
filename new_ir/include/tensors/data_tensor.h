...
class DTensor {
 public:
    ...
    explicit DTensor(const void* data_ptr,
                     uint32_t data_bytes,
                     DataType data_type,
                     int32_t bit_width) {
    ...
    std::shared_ptr<DTensor> clone() {
        return std::shared_ptr<DTensor>(new DTensor(*this));
    }
    ...
    void setStride(std::vector<int64_t> stride) {
        stride_ = stride;
    }

    std::vector<int64_t> getStride() {
        return stride_;
    }

    int32_t getBitWidth() const {
        return bit_width_;
    }

    void setBitWidth(int32_t bit_width) {
        bit_width_ = bit_width;
    }

    void setTensorShape(const STensor shape_tensor) {
        shape_tensor_ = shape_tensor;
    }

    STensor getTensorShape() const {
